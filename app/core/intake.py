...
- LOCATION — this is WHERE THE CANDIDATE must be, NOT where the hiring company is based. A company (US or otherwise) hiring globally or remotely imposes NO candidate-location requirement: NEVER set `location` or `location_country` from the company's HQ/country. Non-US locations are first-class — handle them exactly like US ones. Apply in order:
  ...
  * Remote but SCOPED to a place ("Remote (UK)", "US Remote", "Remote - India", "remote within Europe") is geographically bound, NOT global: set `location_country` (or `location_region`) to that place and keep `remote_ok` FALSE. "Remote (UK)" means remote FROM the UK, not worldwide.
  * TRULY global / "open to candidates anywhere" / no geographic scope at all → leave `location`, `location_country`, and `location_region` empty and set `remote_ok` true. Do NOT infer a region or country from the company.
